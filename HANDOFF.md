...
- Production deployment is pending host identity verification. Strict SSH rejected the target because the stored `38.246.252.84` ECDSA fingerprint is `SHA256:WaSkdMfuMXAYu7DjhNzBkVytdF6d22/6DkUeHbXJKcM`, while repeated read-only scans currently return `SHA256:WLxzk1Nd2TSWUzD8fJm+Y63/dD2JxU7yGX091bZnaBU` (plus stable RSA/ED25519 keys). No host-key checks were bypassed, no backup was created, and no production service or data was changed. Confirm the current server fingerprint through an independent trusted channel before backup/deployment.
- A subsequent read-only SSH attempt using the specified client key path `C:\Users\LEGION\.ssh\sub2api_hk` reached the same strict host-key mismatch and failed before authentication. The private key was not printed or recorded.
