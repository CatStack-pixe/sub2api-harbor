...
describe('AccountUsageCell', () => {
  ...
  it('passes automatic loading and manual refresh through to DeepSeek balance rows', async () => {
    const wrapper = mount(AccountUsageCell, {
      props: {
        account: makeAccount({ id: 6045, platform: 'deepseek', type: 'apikey' }),
        manualRefreshToken: 7,
      },
      global: {
        stubs: {
          DeepSeekBalanceCell: {
            props: ['account', 'autoLoad', 'refreshToken'],
            template: '<div data-test="deepseek-balance">{{ autoLoad }}|{{ refreshToken }}</div>',
          },
        },
      },
    })
    ...
    expect(wrapper.get('[data-test="deepseek-balance"]').text()).toBe('true|7')
    expect(getUsage).not.toHaveBeenCalled()
  })

  it('waits to auto-load a DeepSeek balance while the page is hidden', async () => {
    const wrapper = mount(AccountUsageCell, {
      props: {
        account: makeAccount({ id: 6045, platform: 'deepseek', type: 'apikey' }),
        pageVisible: false,
      ...
      global: {
        stubs: {
          DeepSeekBalanceCell: {
            props: ['account', 'autoLoad', 'refreshToken'],
            template: '<div data-test="deepseek-balance">{{ autoLoad }}</div>',
          },
        },
      },
    })
    await flushPromises()
    expect(wrapper.get('[data-test="deepseek-balance"]').text()).toBe('false')

    await wrapper.setProps({ pageVisible: true })
    await flushPromises()
    expect(wrapper.get('[data-test="deepseek-balance"]').text()).toBe('true')
  })

  it('defers DeepSeek automatic loading on mobile until the row enters the viewport', async () => {
