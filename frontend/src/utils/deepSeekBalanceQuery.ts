...
interface QuerySubscriber {
  resolve: (result: DeepSeekBalanceResult) => void
...
const cache = new Map<number, { result: DeepSeekBalanceResult; expiresAt: number }>()
let activeQueries = 0

const abortError = () => new DOMException('DeepSeek balance query was cancelled', 'AbortError')
...
const settleQuery = (
  query: QueuedQuery,
  result: DeepSeekBalanceResult | undefined,
  error: unknown
) => {
...
const drainQueue = () => {
  while (activeQueries < MAX_CONCURRENT_QUERIES && queue.length > 0) {
    ...
    query.state = 'active'
    activeQueries += 1
    let providerRequest: Promise<DeepSeekBalanceResult>
    try {
      providerRequest = adminAPI.accounts.getDeepSeekBalance(query.accountId, query.controller.signal)
    } catch (error) {
...
export const invalidateDeepSeekBalanceCache = (accountId?: number) => {
  if (accountId == null) cache.clear()
  else cache.delete(accountId)
}

export const queryDeepSeekBalance = (
  accountId: number,
  ...
  const request = new Promise<DeepSeekBalanceResult>((resolve, reject) => {
