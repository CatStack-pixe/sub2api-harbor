...
interface QuerySubscriber {
  resolve: (result: TokenRhythmBalanceResult) => void
...
const cache = new Map<number, { result: TokenRhythmBalanceResult; expiresAt: number }>()
let activeQueries = 0

const abortError = () => new DOMException('TokenRhythm balance query was cancelled', 'AbortError')
...
const settleQuery = (
  query: QueuedQuery,
  result: TokenRhythmBalanceResult | undefined,
  error: unknown
) => {
...
const drainQueue = () => {
  while (activeQueries < MAX_CONCURRENT_QUERIES && queue.length > 0) {
    ...
    query.state = 'active'
    activeQueries += 1
    let providerRequest: Promise<TokenRhythmBalanceResult>
    try {
      providerRequest = adminAPI.accounts.getTokenRhythmBalance(query.accountId, query.controller.signal)
    } catch (error) {
      providerRequest = Promise.reject(error)
    }
    void providerRequest
      .then(
        (result) => settleQuery(query, result, undefined),
        (error) => settleQuery(query, undefined, error)
      )
      .finally(() => {
        activeQueries -= 1
        drainQueue()
...
export const invalidateTokenRhythmBalanceCache = (accountId?: number) => {
  if (accountId == null) cache.clear()
  else cache.delete(accountId)
}

export const queryTokenRhythmBalance = (
  accountId: number,
  options: QueryOptions = {}
): Promise<TokenRhythmBalanceResult> => {
  ...
  const request = new Promise<TokenRhythmBalanceResult>((resolve, reject) => {
