export default {
  ...
  keys: {
    ...
    modelWhitelist: '模型访问限制',
    modelWhitelistHint: '留空表示此密钥可请求分组中的全部模型；配置后只允许匹配的模型。',
    modelRestrictionEnabled: '已配置模型访问限制',
    modelRestrictionSelectGroupFirst: '请先选择分组，加载该分组可访问的模型。',
    modelRestrictionSelected: '已选择 {count} 个模型',
    modelRestrictionSelectAll: '全选',
    modelRestrictionClear: '清除限制',
    modelRestrictionLoading: '正在加载该分组可访问的模型……',
    modelRestrictionLoadFailed: '分组模型加载失败，请重试。',
    modelRestrictionNoModels: '该分组暂无可选择的模型。',
    modelRestrictionUnavailable: '以下已有选项已不在当前分组中：',
    ipRestriction: 'IP 限制',
    ipWhitelist: 'IP 白名单',
