export default {
  promptAudit: {
    ...
    captureOnly: { notice: '仅记录模式会在本机保存适用分组的请求提示词，不发送到审计节点，也不会阻止上游请求。' },
    ...
    policy: {
      ...
      searchGroups: '搜索分组', noGroups: '没有匹配分组', missingGroups: '配置中包含已删除的分组 ID', noGroupsSelected: '当前未选择任何分组，提示词审计不会处理任何分组的请求。', selectedCount: '已选择 {count} 个分组',
      scanners: 'Qwen3Guard 输入风险分类', workerCount: 'Worker 数量', queueCapacity: '持久队列容量', strategy: '节点策略', strategyHint: '按配置顺序优先尝试，必要时故障切换。',
    },
    saveBar: { enabled: '启用提示词审计', captureOnly: '仅记录', blocking: '同步阻止', blockingLatestTurnOnly: '仅审最新输入和上一轮输出', storePass: '保存安全事件', dirty: '有未保存的更改', synced: '配置已同步' },
    blockingConfirm: {
      title: '开启同步阻止？',
    ...
    events: {
	  ...
	  clientIP: '来源 IP',
	  ipNotice: { title: '一次性 IP 通知', placeholder: '输入要在目标下一次 API 调用时显示的信息', send: '发送通知', sending: '发送中…' },
      technical: {
	...
	messages: { saved: '提示词审计配置已保存，明文 API Key 状态已清除。', probeSucceeded: '审计节点连接正常。', deleted: '已删除 {count} 条审计事件。', ipNoticeQueued: '通知已排队，将在 {ip} 的下一次 API 调用时送达。' },
    errors: {
	  loadConfig: '无法加载提示词审计配置。', loadRuntime: '无法加载提示词审计运行态。', loadGroups: '无法加载分组列表。', loadEvents: '无法加载审计事件。', loadDetail: '无法加载事件详情。', saveConfig: '配置保存失败。', probe: '节点探测失败。', delete: '事件删除失败。', previewDelete: '无法生成删除预览，请检查时间范围。', deleteConfirmation: '删除确认无效或已过期，请重新预览。', ipNotice: '通知发送失败。',
	  prompt_audit_event_ip_unavailable: '该历史事件没有可用的来源 IP，无法发送通知。', prompt_audit_ip_notice_invalid_message: '通知内容必须为 1-1000 个字符。',
