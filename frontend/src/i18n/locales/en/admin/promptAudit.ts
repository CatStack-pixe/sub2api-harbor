export default {
  promptAudit: {
    ...
    policy: {
      ...
      searchGroups: 'Search groups', noGroups: 'No matching groups', missingGroups: 'Configured IDs for groups that no longer exist', noGroupsSelected: 'No groups are selected; Prompt Audit will not process requests from any group.', selectedCount: '{count} groups selected',
    ...
    saveBar: { enabled: 'Enable prompt audit', captureOnly: 'Capture only', blocking: 'Synchronous blocking', blockingLatestTurnOnly: 'Only latest input and prior output', storePass: 'Store safe events', dirty: 'Unsaved changes', synced: 'Configuration synced' },
    blockingConfirm: {
    ...
    events: {
	  ...
	  clientIP: 'Source IP',
	  ipNotice: { title: 'One-time IP notice', placeholder: 'Message shown on the target’s next API call', send: 'Send notice', sending: 'Sending…' },
      technical: {
	...
	messages: { saved: 'Prompt Audit configuration saved; plaintext API Key state was cleared.', probeSucceeded: 'The audit node is reachable.', deleted: 'Deleted {count} audit events.', ipNoticeQueued: 'Notice queued for the next API call from {ip}.' },
    errors: {
	  loadConfig: 'Unable to load Prompt Audit configuration.', loadRuntime: 'Unable to load Prompt Audit runtime.', loadGroups: 'Unable to load groups.', loadEvents: 'Unable to load audit events.', loadDetail: 'Unable to load event details.', saveConfig: 'Unable to save the configuration.', probe: 'Node probe failed.', delete: 'Unable to delete events.', previewDelete: 'Unable to create a deletion preview. Check the time range.', deleteConfirmation: 'The deletion confirmation is invalid or expired. Preview again.', ipNotice: 'Unable to queue the notice.',
	  prompt_audit_event_ip_unavailable: 'This historical event has no usable source IP.', prompt_audit_ip_notice_invalid_message: 'The notice must contain 1-1000 characters.',
