...
const baseConfig = (): PromptAuditConfig => ({
  enabled: true, blocking_enabled: false, blocking_latest_turn_only: false, store_pass_events: false, capture_only: false, effective_mode: 'async_audit', strategy: 'priority',
...
describe('PromptAuditView', () => {
  ...
  it('uses capture-only mode without an audit endpoint and forces safe event storage', async () => {
    ...
    await flushPromises()
    await wrapper.get('[data-test="tab-config"]').trigger('click')
    await wrapper.get('[data-test="capture-only-toggle"]').trigger('click')

    expect(wrapper.find('[data-test="endpoint"]').exists()).toBe(false)
    expect(wrapper.find('[data-test="capture-only-notice"]').exists()).toBe(true)
    expect(wrapper.get('[data-test="blocking-toggle"]').attributes()).toHaveProperty('disabled')
