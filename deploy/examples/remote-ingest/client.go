...
type envelope struct { Data json.RawMessage `json:"data"`; Message string `json:"message"` }
...
func post(base, path string, value any, headers map[string]string) json.RawMessage {
	...
	var result envelope
	if err := json.Unmarshal(responseBody, &result); err != nil || response.StatusCode >= 300 { panic(string(responseBody)) }
	return result.Data
}
...
func main() {
	...
	if err != nil { panic(err) }
	var enrolled struct { ClientID string `json:"client_id"` }
	enrollData := post(base, "/api/v1/remote-ingest/enroll", map[string]string{
		"registration_token": os.Getenv("REGISTRATION_TOKEN"), "machine_name": "remote-go-client",
		"public_key": base64.StdEncoding.EncodeToString(publicKey),
	}, headers)
	if err := json.Unmarshal(enrollData, &enrolled); err != nil { panic(err) }
	var challenge struct { ID string `json:"challenge_id"`; Nonce string `json:"nonce"` }
	handshakeData := post(base, "/api/v1/remote-ingest/handshakes", map[string]string{"client_id": enrolled.ClientID}, headers)
	if err := json.Unmarshal(handshakeData, &challenge); err != nil { panic(err) }
	payload := map[string]any{
		"external_id": fmt.Sprintf("remote-%d", time.Now().Unix()), "name": "remote-openai", "platform": "openai",
		"base_url": os.Getenv("REMOTE_BASE_URL"), "api_key": os.Getenv("REMOTE_API_KEY"), "group_name": os.Getenv("REMOTE_GROUP_NAME"),
	...
	for key, value := range headers { requestHeaders[key] = value }
	...
	if err != nil { panic(err) }
	request.Header.Set("Content-Type", "application/json")
	for key, value := range requestHeaders { request.Header.Set(key, value) }
	response, err := (&http.Client{Timeout: 30 * time.Second}).Do(request)
	if err != nil { panic(err) }
	defer response.Body.Close()
	responseBody, _ := io.ReadAll(response.Body)
	if response.StatusCode != http.StatusAccepted { panic(string(responseBody)) }
