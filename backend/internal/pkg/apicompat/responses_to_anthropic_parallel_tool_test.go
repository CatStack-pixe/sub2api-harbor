...
func TestStreamingInterleavedParallelFunctionCallsKeepIndependentLifecycles(t *testing.T) {
	state := NewResponsesEventToAnthropicState()
	...
	feed(ResponsesStreamEvent{
		Type:     "response.created",
		Response: &ResponsesResponse{ID: "resp_interleaved", Model: "gpt-5.2"},
	})
	feed(ResponsesStreamEvent{
		Type:        "response.output_item.added",
		OutputIndex: 3,
	...
	feed(ResponsesStreamEvent{Type: "response.function_call_arguments.delta", OutputIndex: 3, CallID: "call_weather", Delta: `{"city":"`})
	feed(ResponsesStreamEvent{Type: "response.function_call_arguments.delta", OutputIndex: 7, CallID: "call_time", Delta: `{"zone":"`})
	feed(ResponsesStreamEvent{Type: "response.function_call_arguments.delta", OutputIndex: 3, CallID: "call_weather", Delta: `Paris"}`})
	feed(ResponsesStreamEvent{Type: "response.function_call_arguments.done", OutputIndex: 3, CallID: "call_weather", Arguments: `{"city":"Paris"}`})
	feed(ResponsesStreamEvent{Type: "response.function_call_arguments.delta", OutputIndex: 7, CallID: "call_time", Delta: `UTC"}`})
	feed(ResponsesStreamEvent{Type: "response.function_call_arguments.done", OutputIndex: 7, CallID: "call_time", Arguments: `{"zone":"UTC"}`})

	// Upstreams commonly emit output_item.done after arguments.done. These
	// duplicate terminal signals must not create a second stop.
	feed(ResponsesStreamEvent{
	...
	feed(ResponsesStreamEvent{
		Type:        "response.output_item.done",
		OutputIndex: 7,
		Item:        &ResponsesOutput{Type: "function_call", CallID: "call_time", Name: "get_time", Arguments: `{"zone":"UTC"}`},
	})
	feed(ResponsesStreamEvent{
...
func TestStreamingInterleavedParallelReadSanitizationIsPerCall(t *testing.T) {
	...
	feed(ResponsesStreamEvent{Type: "response.function_call_arguments.delta", OutputIndex: 0, Delta: `{"file_path":"/tmp/`})
	feed(ResponsesStreamEvent{Type: "response.function_call_arguments.delta", OutputIndex: 1, Delta: `{"file_path":"/tmp/`})
	feed(ResponsesStreamEvent{Type: "response.function_call_arguments.delta", OutputIndex: 0, Delta: `input.go","pages":""}`})
	feed(ResponsesStreamEvent{Type: "response.function_call_arguments.done", OutputIndex: 0, Arguments: `{"file_path":"/tmp/input.go","pages":""}`})
	feed(ResponsesStreamEvent{Type: "response.function_call_arguments.delta", OutputIndex: 1, Delta: `output.go"}`})
	feed(ResponsesStreamEvent{Type: "response.function_call_arguments.done", OutputIndex: 1, Arguments: `{"file_path":"/tmp/output.go"}`})

	assertIndependentToolLifecycles(t, events, map[string]string{
		"Read":  `{"file_path":"/tmp/input.go"}`,
		"Write": `{"file_path":"/tmp/output.go"}`,
	})
}

type observedToolLifecycle struct {
	name    string
	starts  int
	stops   int
	stopped bool
...
func assertIndependentToolLifecycles(t *testing.T, events []AnthropicStreamEvent, expected map[string]string) {
	t.Helper()
	byBlock := make(map[int]*observedToolLifecycle)
	...
	for _, event := range events {
		...
		case "content_block_start":
			...
			idx := *event.Index
			lifecycle := byBlock[idx]
			if lifecycle == nil {
				lifecycle = &observedToolLifecycle{name: event.ContentBlock.Name}
				byBlock[idx] = lifecycle
			}
			lifecycle.starts++
	...
	require.Len(t, byBlock, len(expected))
	for _, lifecycle := range byBlock {
		require.Contains(t, expected, lifecycle.name)
		require.Equalf(t, 1, lifecycle.starts, "%s must start exactly once", lifecycle.name)
		require.Equalf(t, 1, lifecycle.stops, "%s must stop exactly once", lifecycle.name)
		assert.JSONEq(t, expected[lifecycle.name], lifecycle.args.String(), "%s arguments must be complete", lifecycle.name)
	}
}
