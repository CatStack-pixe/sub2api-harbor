...
func (h *GatewayHandler) Models(c *gin.Context) {
	...
	if platform == service.PlatformComposite {
		...
		writeModelsList(c, service.PlatformComposite, filterAPIKeyModels(apiKey, defaultModelIDsForPlatform(service.PlatformComposite)))
		return
	}

	// Get available models from account configurations for the selected group platform.
	availableModels := h.gatewayService.GetAvailableModels(c.Request.Context(), groupID, platform)
	if apiKey != nil && apiKey.Group != nil && apiKey.Group.RequestModelMappingEnabled() {
		availableModels = groupRequestModelAliases(apiKey.Group)
	...
	if platform == service.PlatformOpenAI {
		writeOpenAIModelsList(c, filterAPIKeyModels(apiKey, defaultModelIDsForPlatform(platform)))
		return
	}

	if platform == service.PlatformGemini {
		writeModelsList(c, platform, filterAPIKeyModels(apiKey, defaultModelIDsForPlatform(platform)))
		return
	}
...
func groupRequestModelAliases(group *service.Group) []string {
	if group == nil || !group.RequestModelMappingEnabled() {
		return nil
	}
	aliases := make([]string, 0, len(group.ModelsListConfig.ModelMapping))
	for alias := range group.ModelsListConfig.ModelMapping {
...
func (h *GatewayHandler) compositeAvailableModels(ctx context.Context, groupID *int64) []string {
	...
	for _, platform := range []string{service.PlatformAnthropic, service.PlatformGemini, service.PlatformOpenAI, service.PlatformAntigravity, service.PlatformGrok, service.PlatformAgnes, service.PlatformDeepSeek, service.PlatformNvidia, service.PlatformTokenRhythm, service.PlatformKimi} {
...
func defaultModelIDsForPlatform(platform string) []string {
	...
	case service.PlatformComposite:
		ids := make([]string, 0)
		seen := make(map[string]struct{})
		for _, concretePlatform := range []string{service.PlatformAnthropic, service.PlatformGemini, service.PlatformOpenAI, service.PlatformAntigravity, service.PlatformGrok, service.PlatformAgnes, service.PlatformDeepSeek, service.PlatformNvidia, service.PlatformTokenRhythm, service.PlatformKimi} {
