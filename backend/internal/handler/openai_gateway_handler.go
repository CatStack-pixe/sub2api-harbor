...
func openAICompatibleRequestPlatform(ctx context.Context, apiKey *service.APIKey) string {
	if platform, ok := service.ResolvedTargetPlatformFromContext(ctx); ok {
		if platform == service.PlatformGrok || platform == service.PlatformAgnes || platform == service.PlatformDeepSeek || platform == service.PlatformNvidia || platform == service.PlatformTokenRhythm || platform == service.PlatformKimi || platform == service.PlatformChatAnywhere || platform == service.PlatformGLM {
			return platform
		}
		return service.PlatformOpenAI
	}
	if apiKey != nil && apiKey.Group != nil && (apiKey.Group.Platform == service.PlatformGrok || apiKey.Group.Platform == service.PlatformAgnes || apiKey.Group.Platform == service.PlatformDeepSeek || apiKey.Group.Platform == service.PlatformNvidia || apiKey.Group.Platform == service.PlatformTokenRhythm || apiKey.Group.Platform == service.PlatformKimi || apiKey.Group.Platform == service.PlatformChatAnywhere || apiKey.Group.Platform == service.PlatformGLM) {
...
func openAICompatibleTextTargetAllowed(c *gin.Context, apiKey *service.APIKey, model string) bool {
	return compositeTargetPlatformAllowed(c, apiKey, model, service.PlatformOpenAI, service.PlatformGrok, service.PlatformAgnes, service.PlatformDeepSeek, service.PlatformNvidia, service.PlatformTokenRhythm, service.PlatformKimi, service.PlatformChatAnywhere, service.PlatformGLM)
}
...
func (h *OpenAIGatewayHandler) Messages(c *gin.Context) {
	...
	if groupMappedModel, matched := resolveGroupRequestModel(apiKey, reqModel); matched {
		routingModel = groupMappedModel
		preferredMappedModel = groupMappedModel
	}
	...
	// 解析渠道级模型映射
	channelMappingMsg, _ := h.gatewayService.ResolveChannelMappingAndRestrict(c.Request.Context(), apiKey.GroupID, routingModel)
	...
	for {
		...
		if err != nil {
			...
			if quotaAccountExhausted {
				h.anthropicStreamingAwareError(c, http.StatusTooManyRequests, "rate_limit_error", "Account request quota exhausted; retry after 24 hours", streamStarted)
				return
			}
...
const (
	...
	// openAISlotAcquireQuotaExhausted：该账号配额已耗尽，调用方应排除账号后重选。
...
func (h *OpenAIGatewayHandler) ResponsesWebSocket(c *gin.Context) {
	...
	for {
		...
		admitWSFollowupTurn := func(turn int) error {
			// turn==1 的会话屏蔽已由握手层检查覆盖；连接内 flag 只拦截后续 turn。
			if cyberBlockedThisConn {
				return service.NewOpenAIWSClientCloseError(coderws.StatusPolicyViolation, cyberSessionBlockedClientMsg, nil)
			}
			// 长连接跨峰谷/倍率刷新防护：每个 turn 按当前时刻重装门并复核
			// 当前账号，越线即要求客户端重连重选。本 turn 的准入与计费共用同一 pricingAt。
			...
			currentUserRelease = wrapReleaseOnDone(ctx, userReleaseFunc)
			currentAccountRelease = wrapReleaseOnDone(ctx, accountReleaseFunc)
			if closeErr := reserveWSRequestQuota(ctx); closeErr != nil {
				releaseTurnSlots()
				return closeErr
			}
			return nil
		...
		hooks := &service.OpenAIWSIngressHooks{
			...
			BeforeTurn:          admitWSFollowupTurn,
			BeforeTurnAdmission: admitWSFollowupTurn,
