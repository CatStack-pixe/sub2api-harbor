...
func (h *RemoteIngestHandler) CreateRegistrationToken(c *gin.Context) {
	...
	if err := c.ShouldBindJSON(&req); err != nil { response.BadRequest(c, "invalid registration token request"); return }
	payload := req
	...
	if result != nil && result.Replayed { c.Header("X-Idempotency-Replayed", "true") }
	raw, err := json.Marshal(result.Data)
	if err != nil { response.ErrorFrom(c, err); return }
	var stored storedRemoteRegistrationToken
	if err := json.Unmarshal(raw, &stored); err != nil { response.ErrorFrom(c, err); return }
...
func (h *RemoteIngestHandler) ListRegistrationTokens(c *gin.Context) {
	...
	for _, item := range items {
		...
		if item.UsedAt != nil { itemStatus = "used" } else if !item.ExpiresAt.After(now) { itemStatus = "expired" }
		if status == "" || status == itemStatus { filtered = append(filtered, item) }
	}
...
func (h *RemoteIngestHandler) ListClients(c *gin.Context) {
	...
	filtered := items[:0]
	for _, item := range items {
		itemStatus := "active"; if item.RevokedAt != nil { itemStatus = "revoked" }
		...
		if matchesSearch && (status == "" || status == itemStatus) { filtered = append(filtered, item) }
	}
	page, pageSize := remotePagination(c)
...
func (h *RemoteIngestHandler) RevokeClient(c *gin.Context) {
	...
	executeAdminIdempotentJSON(c, "admin.remote_ingest.clients.revoke", map[string]string{"client_id": id}, service.DefaultWriteIdempotencyTTL(), func(ctx context.Context) (any, error) {
		if err := h.service.RevokeClient(ctx, id); err != nil { return nil, err }
...
func (h *RemoteIngestHandler) RetryDelivery(c *gin.Context) {
	...
	executeAdminIdempotentJSON(c, "admin.remote_ingest.deliveries.retry", map[string]string{"delivery_id": id}, service.DefaultWriteIdempotencyTTL(), func(ctx context.Context) (any, error) {
		if err := h.service.RetryDelivery(ctx, id); err != nil { return nil, err }
