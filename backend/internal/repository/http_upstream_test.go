...
func TestHTTPUpstreamDoRetriesReplayableRequestThroughConfiguredFallbackProxy(t *testing.T) {
	primaryListener, err := net.Listen("tcp", "127.0.0.1:0")
	require.NoError(t, err)
	primaryURL := "http://" + primaryListener.Addr().String()
	require.NoError(t, primaryListener.Close())
	...
	fallbackRequests := make(chan fallbackRequest, 1)
	fallback := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		fallbackCalls.Add(1)
		body, readErr := io.ReadAll(r.Body)
		if readErr == nil {
			fallbackRequests <- fallbackRequest{body: string(body), uri: r.URL.String()}
		}
		w.WriteHeader(http.StatusOK)
	...
	upstream := NewHTTPUpstream(&config.Config{
		Gateway: config.GatewayConfig{UpstreamProxyFallbackURL: fallback.URL},
	})
	payload := []byte(`{"model":"deepseek-chat","messages":[{"role":"user","content":"ping"}]}`)
	req, err := http.NewRequest(http.MethodPost, "http://api.deepseek.com/v1/chat/completions", bytes.NewReader(payload))
	require.NoError(t, err)

	resp, err := upstream.Do(req, primaryURL, 101, 1)
	require.NoError(t, err)
	require.Equal(t, http.StatusOK, resp.StatusCode)
	require.NoError(t, resp.Body.Close())
	require.Equal(t, int64(1), fallbackCalls.Load())
	select {
	case request := <-fallbackRequests:
		require.Equal(t, string(payload), request.body)
		require.Equal(t, "http://api.deepseek.com/v1/chat/completions", request.uri)
	default:
		require.Fail(t, "expected fallback proxy request")
	}
}

func TestHTTPUpstreamDoDoesNotRetryNonReplayableRequest(t *testing.T) {
	primaryListener, err := net.Listen("tcp", "127.0.0.1:0")
	require.NoError(t, err)
	primaryURL := "http://" + primaryListener.Addr().String()
	require.NoError(t, primaryListener.Close())
	...
	req, err := http.NewRequest(http.MethodPost, "http://api.deepseek.com/v1/chat/completions", io.NopCloser(strings.NewReader("payload")))
	require.NoError(t, err)

	resp, err := upstream.Do(req, primaryURL, 102, 1)
	require.Error(t, err)
	require.Nil(t, resp)
	require.Zero(t, fallbackCalls.Load())
}

func TestHTTPUpstreamDoRetriesProxyGatewayErrorAndReleasesPrimaryEntry(t *testing.T) {
	...
	req, err := http.NewRequest(http.MethodGet, "http://api.deepseek.com/v1/models", nil)
	require.NoError(t, err)

	resp, err := upstream.Do(req, primary.URL, 103, 1)
	require.NoError(t, err)
	require.Equal(t, http.StatusOK, resp.StatusCode)
	require.NoError(t, resp.Body.Close())
	require.Equal(t, int64(1), fallbackCalls.Load())

	primaryKey, _, err := normalizeProxyURL(primary.URL)
	require.NoError(t, err)
	var primaryEntry *upstreamClientEntry
	for _, entry := range upstream.(*httpUpstreamService).clients {
		if entry.proxyKey == primaryKey {
			primaryEntry = entry
			break
		}
	}
	require.NotNil(t, primaryEntry)
	require.Zero(t, atomic.LoadInt64(&primaryEntry.inFlight), "primary entry must be released after gateway-error fallback")
}
