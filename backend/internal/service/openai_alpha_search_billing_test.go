...
func TestCalculateOpenAIRecordUsageCostWebSearchPerCall(t *testing.T) {
	...
	cost, err := svc.calculateOpenAIRecordUsageCostAt(context.Background(), result, apiKey, []string{"gpt-5.6-sol"}, 3.0, 1.0, 1.0, 2.0, UsageTokens{}, "", false, timezone.Now())
	...
	cost, err = svc.calculateOpenAIRecordUsageCostAt(context.Background(), result, apiKey, []string{"gpt-5.6-sol"}, 1.0, 1.0, 1.0, 1.0, UsageTokens{}, "", false, timezone.Now())
	require.NoError(t, err)
	...
	_, err = svc.calculateOpenAIRecordUsageCostAt(context.Background(), result, apiKey, []string{"gpt-5.6-sol"}, 1.0, 1.0, 1.0, 1.0, UsageTokens{InputTokens: 10}, "", false, timezone.Now())
	require.Error(t, err)
}
