...
func TestDetectModelPlatform(t *testing.T) {
	tests := []struct {
		...
		{name: "agnes", model: "agnes-2.0-flash", platform: PlatformAgnes, ok: true},
		...
		{name: "zhipu prefix", model: "zhipu/glm-5.2", platform: PlatformZhipu, ok: true},
		{name: "bigmodel prefix", model: "bigmodel/glm-5.2", platform: PlatformZhipu, ok: true},
...
func TestCompositeGroupSchedulerHasAllCanonicalPlatformBuckets(t *testing.T) {
	...
	require.ElementsMatch(t,
		[]string{PlatformAnthropic, PlatformGemini, PlatformOpenAI, PlatformAntigravity, PlatformGrok, PlatformAgnes, PlatformDeepSeek, PlatformNvidia, PlatformTokenRhythm, PlatformKimi, PlatformZhipu, PlatformChatAnywhere, PlatformGLM, PlatformModelScope, PlatformDashScope, PlatformMiniMax, PlatformVolcengine},
		platforms,
	)
}
