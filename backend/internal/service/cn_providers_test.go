...
func TestProvidersSupportNativeResponses(t *testing.T) {
...
func TestForkAndUpstreamPlatformsRemainValid(t *testing.T) {
	for _, platform := range AllowedQuotaPlatforms {
		t.Run(platform, func(t *testing.T) {
			require.NoError(t, ValidateAccountPlatform(platform))
			require.NoError(t, ValidateGroupPlatform(platform))
			require.True(t, isConcreteRequestPlatform(platform))
		})
	}
	require.NoError(t, ValidateGroupPlatform(PlatformComposite))
	require.Error(t, ValidateAccountPlatform(PlatformComposite))
	require.Error(t, ValidateAccountPlatform("unknown"))

	require.NoError(t, validateAccountCredentials(PlatformOpenCodeGo, AccountTypeAPIKey, map[string]any{"api_key": "sk-test"}))
	require.Error(t, validateAccountCredentials(PlatformOpenCodeGo, AccountTypeAPIKey, nil))
	require.Error(t, validateAccountCredentials(PlatformOpenCodeGo, AccountTypeOAuth, map[string]any{"api_key": "sk-test"}))
}

func TestNativeResponsesSelectionPreservesChatOnlyForkProviders(t *testing.T) {
	for _, platform := range []string{PlatformDeepSeek, PlatformKimi, PlatformMiniMax, PlatformOpenCodeGo} {
