...
func validateAccountCredentials(platform, accountType string, credentials map[string]any) error {
	...
	if platform == PlatformTierflow {
		_, hasCookie := credentials["tierflow_cookie"]
		_, hasUserID := credentials["tierflow_user_id"]
		if hasCookie || hasUserID {
			if _, _, err := TierflowCookieCredentials(credentials); err != nil {
				return infraerrors.BadRequest("TIERFLOW_COOKIE_INVALID", err.Error())
			}
		}
	}
	if platform == PlatformTokenRhythm {
