...
func expectedGroupLifecycleBuckets(groupID int64) []SchedulerBucket {
	platforms := []string{PlatformAnthropic, PlatformGemini, PlatformOpenAI, PlatformAntigravity, PlatformGrok, PlatformAgnes, PlatformDeepSeek, PlatformNvidia, PlatformTokenRhythm}
...
func expectedSchedulerCanonicalBucketCount() int {
	return len(expectedGroupLifecycleBuckets(1))
}

func expectedSchedulerPlatformQueryCount() int {
...
func TestSchedulerGroupLifecycleActiveReopensAndRebuildsAllCurrentBuckets(t *testing.T) {
	...
	accounts.beforeLoad = func() {
		...
		require.Equal(t, expectedSchedulerCanonicalBucketCount(), tokenCount, "all reopen tokens must be prepared before the first account query")
	}
	svc := newGroupLifecycleTestService(cache, accounts, groups, config.RunModeStandard)
	seen := make(map[batchSeenKey]struct{})
	...
	require.Len(t, reopenHeld, expectedSchedulerCanonicalBucketCount())
	for _, held := range reopenHeld {
		require.True(t, held)
	}
	lockTTLs, unlockCalls := cache.lockStats()
	require.Len(t, lockTTLs, expectedSchedulerCanonicalBucketCount())
	for _, ttl := range lockTTLs {
		require.Equal(t, 30*time.Second, ttl)
	}
	require.Equal(t, expectedSchedulerCanonicalBucketCount(), unlockCalls)
	requireLifecycleSeen(t, seen, groupID)
}
