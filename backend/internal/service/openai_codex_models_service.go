...
// CodexModelsManifest is retained as a source-compatible alias for gateway
// callers while the shared OpenAI model response now also represents /models.
...
func (s *OpenAIGatewayService) BuildGroupConfiguredCodexModelsManifest(
	...
	selectedModels, selectionEnabled := codexGroupCatalogSelection(group)
	...
	if group.ModelAllowlistEnabled() {
		body, err = filterCodexModelsByGroupAllowlist(body, group)
		if err != nil {
			return nil, false, fmt.Errorf("filter group Codex models: %w", err)
		}
	}
...
func (s *OpenAIGatewayService) MergeGroupConfiguredCodexModels(
	...
	if group.ModelAllowlistEnabled() {
		...
		body, err = filterCodexModelsByGroupAllowlist(body, group)
		if err != nil {
			return fmt.Errorf("filter group Codex models: %w", err)
		}
		changed = changed || !bytes.Equal(beforeFilter, body)
	}
...
func loadCodexGroupCatalogAccounts(ctx context.Context, repo AccountRepository, groupID int64) (visible []Account, catalog []Account, err error) {
	...
	groupAccounts, listErr := repo.ListModelAvailabilityCandidates(
		...
		[]string{
			...
			PlatformSenseNova, PlatformOpenCodeGo,
		},
		false,
	)
	if listErr != nil {
...
func openAIConfiguredCodexModelIDsForGroup(accounts []Account, group *Group) []string {
	...
	if group.CustomModelsListEnabled() {
		selectedModels = append(selectedModels, group.ModelsListConfig.Models...)
	}
	if group.ModelAllowlistEnabled() {
		selectedModels = append(selectedModels, group.ModelAllowlist.Models...)
	}
...
// A legacy custom catalog still chooses public names and explicit auto modes.
// The allowlist is also used for selection when no custom catalog is enabled.
func codexGroupCatalogSelection(group *Group) ([]string, bool) {
	if group == nil {
		return nil, false
	}
	if group.CustomModelsListEnabled() {
	...
	return group.ModelAllowlist.Models, group.ModelAllowlistEnabled()
}

func filterCodexModelsByGroupAllowlist(body []byte, group *Group) ([]byte, error) {
	if group == nil || !group.ModelAllowlistEnabled() {
		return body, nil
	}
