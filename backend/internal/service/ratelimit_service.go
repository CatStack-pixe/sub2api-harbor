...
func (s *RateLimitService) tryPoolModeInsufficientBalance(ctx context.Context, account *Account, statusCode int, responseBody []byte) bool {
	if s == nil || account == nil || account.Type != AccountTypeAPIKey ||
	...
	messageLower := strings.ToLower(extractUpstreamErrorMessage(responseBody))
	if !strings.Contains(bodyLower, "insufficient account balance") && !strings.Contains(messageLower, "insufficient account balance") {
		return false
	}
...
func (s *RateLimitService) HandleUpstreamError(ctx context.Context, account *Account, statusCode int, headers http.Header, responseBody []byte, requestedModel ...string) (shouldDisable bool) {
	...
	if s.HandleChatAnywhereFreePointsQuotaError(ctx, account, statusCode, "", responseBody) {
		return true
	}
