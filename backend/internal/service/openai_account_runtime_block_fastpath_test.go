...
func TestRuntimeBlockSurvivesStaleProviderSnapshotUntilExplicitRecovery(t *testing.T) {
	for _, platform := range accountPlatformsForGroupPlatform(PlatformOpenAI) {
		t.Run(platform, func(t *testing.T) {
			svc := &OpenAIGatewayService{}
			account := &Account{ID: 92, Platform: platform, Type: AccountTypeAPIKey, Status: StatusActive, Schedulable: true}
			// A candidate snapshot loaded before the cooldown write still has no
			// persisted deadline. It must not reopen the account immediately.
			svc.BlockAccountScheduling(account, time.Now().Add(5*time.Minute), "429")
			require.True(t, svc.isOpenAIAccountRequestRuntimeBlocked(account, "test-model"))
			require.True(t, svc.isOpenAIAccountRuntimeBlocked(account))
			svc.ClearAccountSchedulingBlock(account.ID)
			require.False(t, svc.isOpenAIAccountRequestRuntimeBlocked(account, "test-model"))
		})
	}
}
...
func TestShouldStopOpenAIOAuth429Failover_DefersGrokFailoverLimitToHandler(t *testing.T) {
	svc := &OpenAIGatewayService{}
	...
	require.False(t, svc.ShouldStopOpenAIOAuth429Failover(account, http.StatusTooManyRequests, 1))
	require.False(t, svc.ShouldStopOpenAIOAuth429Failover(account, http.StatusInternalServerError, 2))
	require.False(t, svc.ShouldStopOpenAIOAuth429Failover(account, http.StatusBadGateway, 30))
	require.False(t, svc.ShouldStopOpenAIOAuth429Failover(apiKeyAccount, http.StatusInternalServerError, 2))
