...
type APIKey struct {
	...
	// Allowed request models; empty means all models
...
func (*APIKey) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case apikey.FieldIPWhitelist, apikey.FieldIPBlacklist, apikey.FieldModelWhitelist:
			values[i] = new([]byte)
...
func (_m *APIKey) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case apikey.FieldModelWhitelist:
			if value, ok := values[i].(*[]byte); !ok {
